...
const res = await fetch(process.env.NEXT_PUBLIC_API_URL+AUTEUR_API, { cache:
'no-store' },{ method: 'GET'});
const response = await res.json();
return response;
}
export const fetchAuteurById=async(auteurId)=> {
const res = await fetch(process.env.NEXT_PUBLIC_API_URL+AUTEUR_API+`${auteurId}`, { cache: 'no-store' },{ method: 'GET'
});
const response = await res.json();
return response;
}
