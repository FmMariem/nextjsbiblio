...
const res = await fetch(process.env.NEXT_PUBLIC_API_URL+SPECIALITE_API, { cache:
'no-store' })
...
fetch(process.env.NEXT_PUBLIC_API_URL+SPECIALITE_API+`${specialiteId}`, { cache: 'no-store' },{ method: 'GET'
});
const response = await res.json();
return response;
}
